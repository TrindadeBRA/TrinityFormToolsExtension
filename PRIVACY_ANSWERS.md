...
A extensão TrinityForm tem um único propósito: permitir que desenvolvedores web preencham rapidamente campos de formulário com dados de teste brasileiros válidos (CPF, CNPJ, nome, e-mail e telefone) através do menu de contexto (botão direito). Toda a funcionalidade está focada exclusivamente neste objetivo.
...
**Uso específico:**
- Quando o usuário clica com botão direito em um campo editável e seleciona uma opção (CPF, CNPJ, nome, e-mail ou telefone)
...
**Uso específico:**
- Cria 5 itens no menu de contexto que aparecem apenas em campos editáveis:
  - "Inserir CPF"
  - "Inserir CNPJ"
  - "Inserir Email"
  - "Inserir Telefone"
...
Todas as operações (geração de CPF, CNPJ, nomes, e-mails e telefones) são realizadas localmente no navegador do usuário.
...
- ✅ Os dados gerados (CPF, CNPJ, nome, e-mail, telefone) são criados localmente usando algoritmos JavaScript
...
- Geração de dados de teste (CPF, CNPJ, nome, e-mail, telefone) acontece localmente usando algoritmos JavaScript
...
- Toda geração de dados (CPF, CNPJ, etc.) usa algoritmos matemáticos locais sem necessidade de acesso externo
